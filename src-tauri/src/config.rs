use serde::{Deserialize, Serialize};
use std::fs;
use std::io::Write;
#[cfg(unix)]
use std::os::unix::fs::{OpenOptionsExt, PermissionsExt};
use std::path::{Path, PathBuf};

const DEFAULT_GCP_PROJECT: &str = "project-ced3b331-e814-4d72-8bc";
const DEFAULT_GCP_LOCATION: &str = "europe-west2";
const DEFAULT_LLM_BASE: &str = "https://api.deepseek.com";
const DEFAULT_LLM_MODEL: &str = "deepseek-v4-flash";
const DEFAULT_XAI_BASE: &str = "https://api.x.ai/v1";
const DEFAULT_XAI_MODEL: &str = "grok-4.3";
const PREFERRED_LOCAL_WHISPER_MODEL: &str = "ggml-small.en.bin";

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct AppConfig {
    pub stt_provider: String,
    pub groq_api_key: String,
    pub groq_stt_model: String,
    pub local_whisper_model_path: String,
    pub gcp_project_id: String,
    pub gcp_location: String,
    pub stt_model: String,
    pub llm_provider: String,
    pub llm_base_url: String,
    pub llm_model: String,
    pub llm_api_key: String,
    /// Legacy field kept for migration only; unused when DeepSeek is configured.
    #[serde(default)]
    pub openai_api_key: String,
    pub hotkey: String,
    pub correct_english: bool,
    pub whisper_model: String,
    pub correction_model: String,
    pub hands_free: bool,
    /// Soft start/stop cues when recording begins and ends.
    pub interaction_sounds: bool,
    pub language: String,
    pub active_style_id: String,
    pub app_aware_tone: bool,
    /// Absolute path to the Vibe Coding project root (contains context/, skills/, constitutions/).
    pub vibe_project_root: String,
    /// `local` = Mac STT + Mac LLM; `cloud` = Cloud Run STT + LLM; `hybrid` = Mac STT + Cloud Run LLM.
    pub processing_mode: String,
    pub flow_api_url: String,
    pub flow_api_key: String,
}

impl Default for AppConfig {
    fn default() -> Self {
        let llm_provider = {
            let provider = resolve_env_key("LLM_PROVIDER");
            if provider.is_empty() {
                "deepseek".to_string()
            } else {
                provider
            }
        };
        let (llm_base_url, llm_model) = default_llm_endpoint(&llm_provider);
        Self {
            stt_provider: {
                let provider = resolve_env_key("STT_PROVIDER");
                if provider.is_empty() {
                    "local_whisper".into()
                } else {
                    provider
                }
            },
            groq_api_key: resolve_env_key("GROQ_API_KEY"),
            groq_stt_model: {
                let model = resolve_env_key("GROQ_STT_MODEL");
                if model.is_empty() {
                    "whisper-large-v3-turbo".into()
                } else {
                    model
                }
            },
            local_whisper_model_path: resolve_env_key("LOCAL_WHISPER_MODEL_PATH"),
            gcp_project_id: DEFAULT_GCP_PROJECT.into(),
            gcp_location: DEFAULT_GCP_LOCATION.into(),
            // europe-west2 supports latest_long + en-GB; latest_short rejects en-US there.
            stt_model: "latest_long".into(),
            llm_provider,
            llm_base_url: {
                let base = resolve_env_key("LLM_BASE_URL");
                if base.is_empty() {
                    llm_base_url.into()
                } else {
                    base
                }
            },
            llm_model: {
                let model = resolve_env_key("LLM_MODEL");
                if model.is_empty() {
                    llm_model.into()
                } else {
                    model
                }
            },
            llm_api_key: resolve_llm_key(&resolve_env_key("LLM_PROVIDER")),
            openai_api_key: String::new(),
            // Vibe Coding shortcut base: fn/Globe.
            hotkey: "Section".into(),
            correct_english: true,
            whisper_model: "latest_long".into(),
            correction_model: {
                let model = resolve_env_key("LLM_MODEL");
                if model.is_empty() {
                    llm_model.into()
                } else {
                    model
                }
            },
            // Hidden fallback only; primary fn controls are hold-style.
            hands_free: true,
            interaction_sounds: true,
            language: "en".into(),
            active_style_id: "clear".into(),
            app_aware_tone: true,
            vibe_project_root: String::new(),
            processing_mode: default_processing_mode(),
            flow_api_url: resolve_env_key("FLOW_API_URL"),
            flow_api_key: resolve_env_key("FLOW_API_KEY"),
        }
    }
}

fn default_llm_endpoint(provider: &str) -> (&'static str, &'static str) {
    match provider.trim() {
        "xai" => (DEFAULT_XAI_BASE, DEFAULT_XAI_MODEL),
        _ => (DEFAULT_LLM_BASE, DEFAULT_LLM_MODEL),
    }
}

fn default_processing_mode() -> String {
    let mode = resolve_env_key("PROCESSING_MODE")
        .trim()
        .to_ascii_lowercase();
    if matches!(mode.as_str(), "local" | "cloud" | "hybrid") {
        return mode;
    }

    let url = resolve_env_key("FLOW_API_URL");
    let key = resolve_env_key("FLOW_API_KEY");
    if !url.is_empty() && !key.is_empty() {
        "cloud".into()
    } else {
        "local".into()
    }
}

fn resolve_env_key(name: &str) -> String {
    if let Ok(key) = std::env::var(name) {
        if !key.trim().is_empty() {
            return key.trim().to_string();
        }
    }
    if let Ok(dir) = data_dir() {
        if let Ok(raw) = fs::read_to_string(dir.join(".env")) {
            for line in raw.lines() {
                let line = line.trim();
                if line.is_empty() || line.starts_with('#') {
                    continue;
                }
                let prefix = format!("{name}=");
                if let Some(value) = line.strip_prefix(&prefix) {
                    let value = value.trim().trim_matches('"').trim_matches('\'');
                    if !value.is_empty() {
                        return value.to_string();
                    }
                }
            }
        }
    }
    String::new()
}

pub fn data_dir() -> Result<PathBuf, String> {
    let dir = dirs::config_dir()
        .ok_or_else(|| "Could not resolve config directory".to_string())?
        .join("voice-flow");
    fs::create_dir_all(&dir).map_err(|e| e.to_string())?;
    #[cfg(unix)]
    fs::set_permissions(&dir, fs::Permissions::from_mode(0o700)).map_err(|e| e.to_string())?;
    Ok(dir)
}

fn config_path() -> Result<PathBuf, String> {
    Ok(data_dir()?.join("config.json"))
}

pub fn load_config() -> AppConfig {
    let path = match config_path() {
        Ok(p) => p,
        Err(_) => return AppConfig::default(),
    };

    if !path.exists() {
        let cfg = AppConfig::default();
        let _ = save_config(&cfg);
        return cfg;
    }

    let mut cfg = match fs::read_to_string(&path) {
        Ok(raw) => match serde_json::from_str(&raw) {
            Ok(cfg) => cfg,
            Err(_) => {
                quarantine_corrupt(&path);
                return AppConfig::default();
            }
        },
        Err(_) => AppConfig::default(),
    };
    let mut dirty = false;

    if cfg.stt_provider.trim().is_empty() {
        cfg.stt_provider = "local_whisper".into();
        dirty = true;
    }
    if cfg.groq_api_key.trim().is_empty() {
        let from_env = resolve_env_key("GROQ_API_KEY");
        if !from_env.is_empty() {
            cfg.groq_api_key = from_env;
            dirty = true;
        }
    }
    if cfg.groq_stt_model.trim().is_empty() {
        cfg.groq_stt_model = "whisper-large-v3-turbo".into();
        dirty = true;
    }
    if cfg.local_whisper_model_path.trim().is_empty() {
        if let Ok(dir) = data_dir() {
            let model = dir.join("models").join(PREFERRED_LOCAL_WHISPER_MODEL);
            if model.exists() {
                cfg.local_whisper_model_path = model.to_string_lossy().into_owned();
                dirty = true;
            }
        }
    }
    if cfg.gcp_project_id.trim().is_empty() {
        cfg.gcp_project_id = DEFAULT_GCP_PROJECT.into();
        dirty = true;
    }
    if cfg.gcp_location.trim().is_empty() {
        cfg.gcp_location = DEFAULT_GCP_LOCATION.into();
        dirty = true;
    }
    if cfg.stt_model.trim().is_empty() || cfg.stt_model == "latest_short" {
        // Migrate away from latest_short+en-US which fails in europe-west2.
        if cfg.gcp_location == "europe-west2" || cfg.stt_model.trim().is_empty() {
            cfg.stt_model = "latest_long".into();
            dirty = true;
        }
    }
    if cfg.llm_provider.trim().is_empty() {
        cfg.llm_provider = "deepseek".into();
        dirty = true;
    }
    if cfg.llm_provider == "xai" {
        if cfg.llm_base_url.trim().is_empty() || cfg.llm_base_url == DEFAULT_LLM_BASE {
            cfg.llm_base_url = DEFAULT_XAI_BASE.into();
            dirty = true;
        }
        if cfg.llm_model.trim().is_empty() || cfg.llm_model == DEFAULT_LLM_MODEL {
            cfg.llm_model = DEFAULT_XAI_MODEL.into();
            cfg.correction_model = DEFAULT_XAI_MODEL.into();
            dirty = true;
        }
    }
    if cfg.llm_base_url.trim().is_empty() {
        cfg.llm_base_url = DEFAULT_LLM_BASE.into();
        dirty = true;
    }
    if cfg.llm_model.trim().is_empty()
        || cfg.llm_model == "gpt-4o-mini"
        || cfg.correction_model == "gpt-4o-mini"
    {
        cfg.llm_model = DEFAULT_LLM_MODEL.into();
        cfg.correction_model = DEFAULT_LLM_MODEL.into();
        dirty = true;
    }
    // Prefer env/.env when config file has no LLM key yet.
    if cfg.llm_api_key.trim().is_empty() {
        let from_env = resolve_llm_key(&cfg.llm_provider);
        if !from_env.is_empty() {
            cfg.llm_api_key = from_env;
            dirty = true;
        }
    }
    // Env / .env is a *fallback* that fills blanks, never an override. It used to win on
    // every load, which silently reverted anything saved from Hub Settings — picking
    // "local" mode or pasting a different Cloud Run URL appeared to work, then undid
    // itself on the next read.
    let mode = cfg.processing_mode.trim().to_ascii_lowercase();
    if mode.is_empty() {
        cfg.processing_mode = default_processing_mode();
        dirty = true;
    } else if !matches!(mode.as_str(), "local" | "cloud" | "hybrid") {
        cfg.processing_mode = default_processing_mode();
        dirty = true;
    } else if cfg.processing_mode != mode {
        cfg.processing_mode = mode;
        dirty = true;
    }
    if cfg.flow_api_url.trim().is_empty() {
        let url = resolve_env_key("FLOW_API_URL");
        if !url.is_empty() {
            cfg.flow_api_url = url;
            dirty = true;
        }
    }
    if cfg.flow_api_key.trim().is_empty() {
        let key = resolve_env_key("FLOW_API_KEY");
        if !key.is_empty() {
            cfg.flow_api_key = key;
            dirty = true;
        }
    }
    // Do not overwrite an explicit local/hybrid choice just because Cloud Run
    // credentials exist. Hybrid is Mac STT + Cloud LLM; local keeps its own keys.
    // One-time migration: old OpenAI-only configs.
    if cfg.llm_api_key.trim().is_empty() && !cfg.openai_api_key.trim().is_empty() {
        // Keep openai key in file but do not use it for the MyGCP pipeline.
    }

    // Migrate old shortcut labels to the current Vibe Coding fn model.
    if matches!(
        cfg.hotkey.as_str(),
        "Alt+Space" | "Option+Space" | "Command+1" | ""
    ) {
        cfg.hotkey = "Section".into();
        dirty = true;
    }
    if dirty {
        let _ = save_config(&cfg);
    }

    cfg
}

fn resolve_llm_key(provider: &str) -> String {
    match provider.trim() {
        "xai" => resolve_env_key("XAI_API_KEY"),
        "openai_compatible" => resolve_env_key("LLM_API_KEY"),
        _ => {
            let key = resolve_env_key("DEEPSEEK_API_KEY");
            if key.is_empty() {
                resolve_env_key("LLM_API_KEY")
            } else {
                key
            }
        }
    }
}

pub fn save_config(config: &AppConfig) -> Result<(), String> {
    let path = config_path()?;
    let raw = serde_json::to_string_pretty(config).map_err(|e| e.to_string())?;
    atomic_write_private(&path, &raw)
}

pub fn atomic_write_private(path: &Path, contents: &str) -> Result<(), String> {
    let tmp = path.with_extension("tmp");
    {
        let mut options = fs::OpenOptions::new();
        options.create(true).write(true).truncate(true);
        #[cfg(unix)]
        options.mode(0o600);
        let mut file = options
            .open(&tmp)
            .map_err(|e| format!("Could not write {}: {e}", tmp.display()))?;
        file.write_all(contents.as_bytes())
            .map_err(|e| e.to_string())?;
        let _ = file.sync_all();
    }
    #[cfg(unix)]
    fs::set_permissions(&tmp, fs::Permissions::from_mode(0o600)).map_err(|e| e.to_string())?;
    fs::rename(&tmp, path).map_err(|e| format!("Could not replace {}: {e}", path.display()))
}

pub fn quarantine_corrupt(path: &Path) {
    let backup = path.with_extension("json.corrupt");
    let _ = fs::rename(path, backup);
}

pub fn llm_key_configured(config: &AppConfig) -> bool {
    !config.llm_api_key.trim().is_empty()
}

pub fn language_bcp47(language: &str) -> String {
    match language.trim() {
        // Default English → en-GB: required for Speech latest_* models in europe-west2.
        "" | "auto" | "en" => "en-GB".into(),
        "en-us" | "en_US" | "en-US" => "en-US".into(),
        "en-gb" | "en_GB" | "en-GB" => "en-GB".into(),
        other if other.contains('-') => other.to_string(),
        other => other.to_string(),
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn english_defaults_to_en_gb() {
        assert_eq!(language_bcp47("en"), "en-GB");
        assert_eq!(language_bcp47(""), "en-GB");
        assert_eq!(language_bcp47("en-US"), "en-US");
    }

    #[test]
    fn non_english_keeps_the_language_code() {
        assert_eq!(language_bcp47("es"), "es");
        assert_eq!(language_bcp47("fr"), "fr");
        assert_eq!(language_bcp47("de"), "de");
        assert_eq!(language_bcp47("pt"), "pt");
        assert_eq!(language_bcp47("it"), "it");
    }
}
